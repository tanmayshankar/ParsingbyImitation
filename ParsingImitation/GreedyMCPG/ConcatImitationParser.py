...
class Parser():
	...
	def select_split_learner_sample(self):
		...
		if self.state.rule_applied==0:
			lower_lim = float(self.state.x)/(self.data_loader.image_size-1)
			upper_lim = float(self.state.x+self.state.w)/(self.data_loader.image_size-1)
		if self.state.rule_applied==1:
			lower_lim = float(self.state.y)/(self.data_loader.image_size-1)
			upper_lim = float(self.state.y+self.state.h)/(self.data_loader.image_size-1)
		...
		while redo: 
			...
			log_split, self.state.split = self.sess.run([self.model.logitnormal_sample,self.model.sample_split], 
				feed_dict={self.model.input: input_image,
							self.model.lower_lim: npy.reshape(lower_lim,(1,1)),
							self.model.upper_lim: npy.reshape(upper_lim,(1,1))})
		...
		for k in range(len(indices)):
			...
			if state.rule_applied==0 or state.rule_applied==1:
				...
				if state.rule_applied==0:
					self.batch_lower_lims[k] = state.x
					self.batch_upper_lims[k] = state.x+state.w
				if state.rule_applied==1:
					self.batch_lower_lims[k] = state.y
					self.batch_upper_lims[k] = state.y+state.h
		# embed()
		# Call sess train.
