...
class hierarchical():
	...
	def initialize_tensorflow_model(self, sess, model_file=None):
		...
		self.split_dist = tf.contrib.distributions.BetaWithSoftplusConcentration(self.fcs2_preslice[0,0],self.fcs2_preslice[0,1])
		...
		# self.split_dist = tf.contrib.distributions.Normal(loc=self.split_mean,scale=0.1)
		...
		# TRYING SPLIT LOSS WITH NEGATIVE SIGN 30/06: MUST MINIMIZE NEGATIVE LOG PROBABILITY, to maximize log probability / log likelihood. 
		...
		self.multitask_split_loss_weight = 1.
		self.total_loss = tf.add(self.rule_loss,self.multitask_split_loss_weight*self.split_loss,name='total_loss')
		...
		# self.split_cov_summary = tf.summary.scalar('Split_Covariance',self.split_cov)
		# self.split_mean_summary = tf.summary.scalar('Split_Mean',self.split_mean)
