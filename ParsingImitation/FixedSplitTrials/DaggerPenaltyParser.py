...
class Parser():
	...
	def select_split_learner_sample(self):
		...
		self.state.split = 0.6
		log_split = 0.6

		if self.state.rule_applied==0:
			# Split between 0 and 1 as s. 
			# Map to l from x+1 to x+w-1. 
			self.state.boundaryscaled_split = int((self.state.w-2)*log_split+self.state.x+1)
		...
		if self.state.rule_applied==1:
			self.state.boundaryscaled_split = int((self.state.h-2)*log_split+self.state.y+1)
	...
	def backprop(self, iter_num):
		...
		self.batch_rule_masks = npy.ones((self.batch_size,self.model.num_rules))
		...
		for k in range(len(indices)):
			...
			self.batch_target_rules[k,npy.where(state.rule_mask==0)[0]]=-1.
			# self.batch_target_rules[k, state.rule_mas]
