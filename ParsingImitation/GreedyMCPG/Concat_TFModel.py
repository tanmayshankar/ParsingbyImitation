...
class Model():
	...
	def define_split_stream(self):
		...
		# [a,b] values in pixels indices (divided by 255).
		...
		self.fc6_concat = tf.concat([self.fc6,self.lower_lim,self.upper_lim],axis=-1)		
		self.normal_mean = tf.layers.dense(self.fc6_concat,1)
		self.normal_var = tf.layers.dense(self.fc6_concat,1,activation=tf.nn.softplus)
