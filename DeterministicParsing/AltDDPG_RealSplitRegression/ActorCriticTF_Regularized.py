	...
	def initialize_base_model(self, sess, model_file=None, to_train=None, name_scope=None):
		...
		if name_scope:
			...
			with tf.variable_scope(self.name_scope):
				...
				# Initial layer.
				self.conv[0] = tf.layers.conv2d(self.input,filters=self.conv_num_filters[0],kernel_size=(self.conv_sizes[0]),
					strides=(self.conv_strides[0]),activation=tf.nn.relu,name='conv0',kernel_regularizer=tf.contrib.layers.l2_regularizer(self.regularization_coeff))

				# Defining subsequent conv layers.
				for i in range(1,self.num_layers):
					self.conv[i] = tf.layers.conv2d(self.conv[i-1],filters=self.conv_num_filters[i],kernel_size=(self.conv_sizes[i]),
						strides=(self.conv_strides[i]),activation=tf.nn.relu,name='conv{0}'.format(i),kernel_regularizer=tf.contrib.layers.l2_regularizer(self.regularization_coeff))
				...
				self.fc6 = tf.layers.dense(self.flat_conv,self.fc6_shape,activation=tf.nn.relu,name='base_fc6',kernel_regularizer=tf.contrib.layers.l2_regularizer(self.regularization_coeff))
...
class ActorCriticModel():
	...
	def define_critic_train_op(self):
		...
		self.critic_loss = tf.losses.mean_squared_error(self.target_Qvalue, self.critic_network.predicted_Qvalue) + tf.losses.get_regularization_losses(scope=self.critic_network.name_scope)
	...
	def define_actor_train_op(self):
		...
		self.actor_loss = -self.critic_network.predicted_Qvalue + tf.losses.get_regularization_losses(scope=self.actor_network.name_scope)
