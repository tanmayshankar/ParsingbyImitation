...
class GradientNet():
	...
	def __init__(self, sess, base_filepath=''):
		...
		self.image_mean = [ 175.5183833 ,  176.6830765 ,  192.35719172]
		...
		# Compiling the model.
		self.model = keras.models.Model(inputs=self.base_model.input, outputs=self.class_predictions)
	...
	def preprocess(self):
		...
		self.image_gradients = npy.zeros((self.num_images,2,self.image_size))
	...
	def train(self):
		...
		for e in range(1,self.num_epochs+1):
			...
			for i in range(self.num_images/self.batch_size):
				...
				indices = index_list[i*batch_size:(i+1)*batch_size]
				self.batch_inputs = self.images[[indices]]
				# Picking up HORIZONTAL GRADIENTS now
				self.batch_targets = self.image_gradients[[indices],0]
	...
	def forward(self, epoch):
		...
		npy.save("Predicted_gradients_{0}.npy".format(e),self.predicted_gradients)
