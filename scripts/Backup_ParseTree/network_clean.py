...
class hierarchical():
			...
			rule_loss, split_loss, start_loss, goal_loss, startgoal_loss, _ = self.sess.run([self.rule_loss, self.split_loss, self.start_loss,self.goal_loss,self.startgoal_loss, self.train], \
				feed_dict={self.input: self.resized_image.reshape(1,self.image_size,self.image_size,1), self.sampled_split: self.parse_tree[j].split, \
					self.sampled_goal: self.parse_tree[j].goal, self.sampled_start: self.parse_tree[j].start, \
						self.previous_goal: previous_goal, self.rule_return_weight: rule_weight, self.split_return_weight: split_weight, \
							self.startgoal_return_weight: startgoal_weight, self.target_rule: target_rule})
	...
	def construct_parse_tree(self,image_index):
		...
		while ((self.predicted_labels[image_index]==0).any() or (self.current_parsing_index<=len(self.parse_tree)-1)):
			...
			plt.imshow(self.predicted_labels[image_index])
			plt.colorbar()
			plt.show()			
