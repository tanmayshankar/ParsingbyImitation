...
class hierarchical():
	...
	def initialize_tensorflow_model(self, sess, model_file=None):
		...
		self.split_dist = tf.contrib.distributions.Categorical(probs=self.categorical_probabilities,allow_nan_stats=False)
		...
		if selected_rule<=3:
			...
			if ((selected_rule==0) or (selected_rule==2)):
				...
				while (split_location<=0)or(split_location>=self.state.h):				
					...
					# Setting action probabilities from both policies.
					grad_pol_prob = categorical_prob_softmax[split_location]
					net_pol_prob = split_probabilities[split_location]
			...
			# Storing probabilities in state.
			self.parse_tree[self.current_parsing_index].gradient_policy_probability = grad_pol_prob
			self.parse_tree[self.current_parsing_index].network_policy_probability = net_pol_prob
		...
		for j in range(len(self.parse_tree)):
			...
			if self.parse_tree[j].label==0:
				...
				if self.parse_tree[j].rule_applied<=3:
					...
					split_weight *= self.parse_tree[j].network_policy_probability/self.parse_tree[j].gradient_policy_probability				
