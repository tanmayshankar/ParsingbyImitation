...
class GradientNet():
	...
	def __init__(self, sess):
		...
		# x = keras.layers.Conv2DTranspose(filters=20,kernel_size=4,strides=2, activation='relu',padding='SAME')(x)
		# print(x._keras_shape)
		# x = keras.layers.Conv2DTranspose(filters=2,kernel_size=32,strides=16,activation='relu',padding='SAME')(x)

		x = keras.layers.Conv2DTranspose(filters=20,kernel_size=4,strides=2, activation='relu',padding='VALID')(x)
		print(x._keras_shape)
		x = keras.layers.Conv2DTranspose(filters=20,kernel_size=4,strides=2,activation='relu',padding='VALID')(x)
		print(x._keras_shape)
		x = keras.layers.Conv2DTranspose(filters=2,kernel_size=24,strides=8,activation='relu',padding='VALID')(x)
		...
		self.lambda_grad = keras.layers.Lambda(lambda y: dummy_func(y))(x)
		# self.lambda_grad = keras.layers.Lambda(lambda y: dummy_func(y))(x1)
