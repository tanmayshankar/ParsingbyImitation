...
class Parser():
	...
	def __init__(self, model_instance=None, data_loader_instance=None, memory_instance=None, plot_manager=None, args=None, session=None): 
		...
		self.max_number_nodes = 2**(self.max_depth)
		for k in range(self.max_depth+1):
			self.max_number_nodes += 2**k
		# SHould be 23 for 4.
	...
	def append_parse_tree(self):
		for k in range(len(self.parse_tree)):
			...
			if self.parse_tree[k].rule_applied==0 or self.parse_tree[k].rule_applied==1:
				self.memory.append_to_memory(self.parse_tree[k],k,self.parse_tree)
	...
	def select_rule_learner_greedy(self):
		...
		rule_probabilities = self.sess.run(self.model.rule_probabilities, feed_dict={self.model.input: input_image,
				self.model.tree_embedding: self.parse_tree_embedding.reshape((1,self.max_number_nodes*4)),
	...
	def construct_parse_tree(self, image_index):
		...
		# Construct parse tree embedding.

		self.parse_tree_embedding = npy.zeros((self.max_number_nodes*4))
		...
		while ((self.predicted_labels[image_index]==0).any() or (self.current_parsing_index<=len(self.parse_tree)-1)):					
			...
			# X, Y, W, H.
			# UPDATE PARSE TREE EMBEDDING.
			if 4*(self.current_parsing_index+1)>92:
				embed()
			self.parse_tree_embedding[(4*self.current_parsing_index):(4*(self.current_parsing_index+1))] = npy.array([self.state.x,self.state.y,self.state.w,self.state.h])
	...
	def backprop(self, iter_num):
		...
		self.batch_tree_embeddings = npy.zeros((self.batch_size,self.max_number_nodes*4))
		...
		for k in range(len(indices)):
			...
			state, k_value, parse_tree = copy.deepcopy(self.memory.memory[indices[k]])
			...
			for jx in range(k_value):			
				# self.batch_tree_embeddings[k, :((k_value+1)*4)] = parse_tree[jx]
				self.batch_tree_embeddings[k, (4*jx):((jx+1)*4)] = npy.array([parse_tree[jx].x,parse_tree[jx].y,parse_tree[jx].w,parse_tree[jx].h])
		...
		self.sess.run(self.model.train, feed_dict={self.model.input: self.batch_states,
								...
								self.model.tree_embedding: self.batch_tree_embeddings})
