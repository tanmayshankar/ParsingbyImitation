...
class hierarchical():
	...
	def alternate_propagate_rewards(self):
		...
		for j in reversed(range(len(self.parse_tree))):
			if (self.parse_tree[j].backward_index>=0):
				self.parse_tree[self.parse_tree[j].backward_index].penalty += self.parse_tree[j].intermittent_term

		for j in range(len(self.parse_tree)):
			self.parse_tree[j].reward += self.intermittent_lambda*self.parse_tree[j].penalty
