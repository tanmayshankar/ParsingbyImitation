...
class hierarchical():
	...
	def __init__(self):
		...
		self.paintwidth = int(sys.argv[4])
	...
	def initialize_tensorflow_model(self, sess, model_file=None):
		...
		self.split_alpha = tf.nn.softplus(self.fcs2_preslice[0,0])
		self.split_beta = tf.nn.softplus(self.fcs2_preslice[0,1])
		...
		self.split_dist = tf.contrib.distributions.Beta(concentration1=self.split_alpha,concentration0=self.split_beta,name='split_beta_dist')
		...
		if selected_rule<=3:
			...
			if ((selected_rule==0) or (selected_rule==2)):
				...
				while (split_location<=0)or(split_location>=self.state.h):
					...
					inter_split = split_location*(self.imagey-2)
			...
			if ((selected_rule==1) or (selected_rule==3)):
				...
				while (split_location<=0)or(split_location>=self.state.w):
					...
					inter_split = split_location*(self.imagex-2)
