...
class hierarchical():
	...
	def parse_primitive_terminal(self, image_index):
		...
		if (self.state.label==1):
			...
			self.start_copy = copy.deepcopy(start_location)
			self.goal_copy = copy.deepcopy(goal_location)
			# start_location *= npy.array([self.state.w,self.state.h])
			# goal_location *= npy.array([self.state.w,self.state.h])

			self.current_start = npy.array([self.state.w*start_location[0],self.state.h*start_location[1]])
			# self.previous_goal = copy.deepcopy(self.current_goal)
			self.current_goal = npy.array([self.state.w*goal_location[0],self.state.h*goal_location[1]])		

			self.parse_tree[self.current_parsing_index].start = self.current_start
			self.parse_tree[self.current_parsing_index].goal = self.current_goal
