...
class Parser():
	...
	def backprop(self, iter_num):
		...
		for k in range(len(indices)):
			...
			# If split rule is being applied, set batch_selected_splits as well.
		...
		# In One Step Temporal Difference Actor Critic, here's what we needed to do: 
		# Compute a_{t+1}, as forward pass of actor \mu(s_{t+1}), so that we could 
		# evaluate Critic Q(s_{t+1},\mu(s_{t+1})) to bootstrap (for TD error). 

		# Here, since we're following Episodic task, with Monte Carlo samples of the RETURN, 
		# We do not need this. We directly have samples of Q_t (G_t). 
		# We don't need forward pass of actor on s_{t+1} or forward pass of critic. 

		# The change is that here we will have to FEED in the action taken 
		# as input to the Critic network, instead of letting it take these as input from 
		# the actor network directly. 
		# This is because we're evaluating Q(s_t,a_t) (in the Critic loss), rather than 
		# Q(s_t, \mu_(s_t | \theta)). 

		# UPDATE CRITIC FIRST: 
		# MUST FORWARD PASS CRITIC FOR STOCHASTIC PART OF ACTOR UPDATE. 	
		critic_estimates, _ = self.sess.run([self.ACModel.critic_network.predicted_Qvalue,self.ACModel.train_critic],
		...
		_ = self.sess.run(self.ACModel.train_actor,
			feed_dict={self.ACModel.actor_network.split_weight: self.batch_split_weights,
						...
						self.ACModel.actor_network.rule_return_weight: critic_estimates,
						# Pre-calculating critic_estimates for this part of the return means we can
						# feed whatever we want (i.e. what's below) to the critic without
						# screwing up the stochastic part of the actor's update.
													
						self.ACModel.critic_network.input: self.batch_states,
						self.ACModel.critic_network.selected_rule: self.batch_target_rules,

						# NOW feeding in the actor_splits instead of batch_selected_splits.
						self.ACModel.critic_network.selected_split: actor_splits})
