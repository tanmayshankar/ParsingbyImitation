...
class ModularNet():
	...
	# 		self.sc3.set_data(self.images[image_index,:,:,2])
	...
	def update_plot_data(self, image_index):
		...
		if self.plot:
			...
			self.hprob_manager[0].set_data(self.h_probs,range(255))
			self.vprob_manager[0].set_data(range(255),255*self.v_probs)
			# embed()
			# Plotting split line segments from the parse tree.
			split_segs = []
			for j in range(len(self.parse_tree)):
			...
			self.split_lines = self.ax[0,1].add_collection(split_lines)				
			self.split_lines2 = self.ax[0,2].add_collection(split_lines2)
			self.split_lines3 = self.ax[1,1].add_collection(split_lines3)
			...
			if len(self.start_list)>0 and len(self.goal_list)>0:
				...
				lines = LineCollection(segs, colors=color_index,linewidths=linewidths)
				self.lines = self.ax[0,0].add_collection(lines)	
			...
			if len(self.ax[0,0].collections):
				del self.ax[0,0].collections[-1]
		
			del self.ax[1,1].collections[-1]
			del self.ax[0,2].collections[-1]			
			del self.ax[0,1].collections[-1]
	...
	def define_plots(self):
		...
		if self.plot:
			...
			self.sc1 = self.ax[0,0].imshow(self.predicted_labels[image_index],aspect='equal',cmap='jet',extent=[0,self.image_size,0,self.image_size],origin='lower')
			# self.sc1 = self.ax[0,0].imshow(self.predicted_labels[image_index],aspect='equal',cmap='jet')
			self.sc1.set_clim([-1,1])
			# self.sc1.set_clim([0,2])
			self.ax[0,0].set_title("Predicted Labels")
			self.ax[0,0].set_adjustable('box-forced')
			# self.ax[0,0].set_xlim(self.ax[0,0].get_xlim()[0]-0.5, self.ax[0,0].get_xlim()[1]+0.5) 
			# self.ax[0,0].set_ylim(self.ax[0,0].get_ylim()[0]-0.5, self.ax[0,0].get_ylim()[1]+0.5) 

			self.sc2 = self.ax[0,1].imshow(self.true_labels[image_index],aspect='equal',cmap='jet',extent=[0,self.image_size,0,self.image_size],origin='lower')
			# self.sc2 = self.ax[0,1].imshow(self.true_labels[image_index],aspect='equal',cmap='jet')
			self.sc2.set_clim([-1,1])
			self.ax[0,1].set_title("Parse Tree")
			self.ax[0,1].set_adjustable('box-forced')

			self.sc3 = self.ax[0,2].imshow(self.images[image_index,:,:,0],aspect='equal',cmap='jet',extent=[0,self.image_size,0,self.image_size],origin='lower')
			# self.sc3 = self.ax[0,2].imshow(self.images[image_index],aspect='equal',cmap='jet')
			self.sc3.set_clim([0,255])
			self.ax[0,2].set_title("Actual Image")
			self.ax[0,2].set_adjustable('box-forced')

			self.sc4 = self.ax[1,1].imshow(self.true_labels[image_index],aspect='equal',cmap='jet',extent=[0,self.image_size,0,self.image_size],origin='lower')
			# self.sc4 = self.ax[1,1].imshow(self.true_labels[image_index],aspect='equal',cmap='jet') #, extent=[0,self.image_size,0,self.image_size],origin='lower')
			self.sc4.set_clim([-1,1])
	...
	def preprocess(self):
		...
		for i in range(self.num_images):
			for k in range(3):
				self.images[i,:,:,k] = self.binary_images[i]
...
def main(args):
	...
	hierarchical_model.binary_images = npy.load(args.images)
