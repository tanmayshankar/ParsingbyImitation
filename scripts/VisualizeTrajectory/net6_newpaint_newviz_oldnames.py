...
class hierarchical():
	...
	def parse_primitive_terminal(self, image_index):
		...
		if (self.state.label==1):
			...
			# selected_primitive = npy.random.choice(range(self.number_plsrimitives),p=primitive_probabilities[0])
			...
			if (selected_primitive==2):
				...
				self.painted_image[image_index, self.state.x+(self.state.w-self.paintwidth)/2:self.state.x+(self.state.w+self.paintwidth)/2, self.state.y:self.state.y+self.state.h] = 1.
