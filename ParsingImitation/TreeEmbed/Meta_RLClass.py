...
import TreeEmbedTF
import Data_Loader
import TreeEmbedParser
import NewPlotting
import TreeEmbedMemory
...
class Meta_RLClass():
	...
	def __init__(self, session=None,arguments=None):
		...
		self.model = TreeEmbedTF.Model(num_channels=self.data_loader.num_channels)		
		...
		# Instantiate memory. 
		self.memory = TreeEmbedMemory.Replay_Memory()
		...
		self.parser = TreeEmbedParser.Parser(model_instance=self.model, data_loader_instance=self.data_loader,
