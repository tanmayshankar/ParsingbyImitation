	...
	def append_parse_tree(self):
		for k in range(len(self.parse_tree)):
			...
			if self.parse_tree[k].label==0:
				...
				#############################################################
				#############################################################
				###### Only adding transitions EXACTLY at switch depth. #####
				###### This is very important. 							#####
				#############################################################
				#############################################################

				if self.parse_tree[k].depth==self.switch_depth:
		...
		while ((self.predicted_labels[image_index]==0).any() or (self.current_parsing_index<=len(self.parse_tree)-1)):					
			...
			if self.state.label==0:
				...
				if self.args.train:
					...
					# If less than switch depth, act according to a mixture of the policies. 
					...
					# If we're at the switch depth of more, act according to the expert policy. 
					else: 
						self.parse_nonterminal_offpolicy()
				else:
					# Using Learnt Policy
