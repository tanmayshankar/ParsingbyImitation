...
class ActorModel():
	...
	def define_base_model(self, sess, model_file=None, to_train=None):
		...
		# Starting regularization_coeff at 0.001 now.
	...
	def define_split_stream(self):
		...
		# Changing initialization value to +/- 3 instead of 3e-4
...
class CriticModel():
	...
	def define_eval_stream(self):
		...
		# Changing initialization_val to 3 from 3e-4
		self.initialization_val = 1
		
		# Concatenate the image features with the predicted split and selected rule. 
		# Except now here actor_split and onehot_rules are going to be placeholders. 

		# Placeholder for selected split. 
		self.selected_split = tf.placeholder(tf.float32,shape=(None,1),name='selected_split')
		# One-hot Placeholder for selected rule. 		
		self.selected_rule = tf.placeholder(tf.float32,shape=(None,self.num_rules),name='selected_rule')		
		self.concat_input = tf.concat([self.fc8, self.selected_split, self.selected_rule],axis=-1,name='concat')
...
class ActorCriticModel():
	...
	def define_actor_train_op(self):
		...
		# Critic gradients with respect to the chosen split location. 		
		self.critic_gradients_wrt_split = tf.gradients(-self.critic_network.predicted_Qvalue,self.critic_network.selected_split)
		...
		# Actor gradients due to split stream. 
	...
	def define_logging_ops(self):
		...
		# self.selected_rule_summary = tf.summary.scalar('Selected_Rule',tf.)
		# self.selected_split_summary = 
