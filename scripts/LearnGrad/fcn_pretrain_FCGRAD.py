...
# For object images we collected:
...
class FCN16VGG:
	...
	def build(self, train=False, num_classes=20, random_init_fc8=False,
		...
		self.expanded_input = tf.expand_dims(self.input_image,0)
		...
		self.vertical_image_gradients = tf.placeholder(tf.float32,shape=(256))

		self.hloss = tf.nn.softmax_cross_entropy_with_logits(labels=self.horizontal_image_gradients, logits=self.horizontal_grad_presf)
		self.vloss = tf.nn.softmax_cross_entropy_with_logits(labels=self.vertical_image_gradients, logits=self.vertical_grad_presf)

		self.total_loss = self.hloss + self.vloss
