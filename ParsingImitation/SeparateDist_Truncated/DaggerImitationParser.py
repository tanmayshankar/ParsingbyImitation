...
class Parser():
	...
	def select_split_expert_greedy(self):
		...
		if self.state.rule_applied==0:
			...
			self.state.split = float(self.state.boundaryscaled_split+self.state.x)/(self.data_loader.image_size-1)
		...
		if self.state.rule_applied==1:	
			self.state.split = float(self.state.boundaryscaled_split+self.state.y)/(self.data_loader.image_size-1)
			# Must add resultant states to parse tree.
	...
	def select_split_learner_sample(self):
		...
		# Set a_val, b_val and run the split_mean and split_cov based on which distribution we use. 
		if self.state.rule_applied==0:
			...
			split_mean, split_cov = self.sess.run([self.model.horizontal_normal_mean,self.model.horizontal_normal_var], feed_dict={self.model.input: input_image})
		...
		if self.state.rule_applied==1:
			...
			split_mean, split_cov = self.sess.run([self.model.vertical_normal_mean,self.model.vertical_normal_var], feed_dict={self.model.input: input_image})
		
		# Adjusting things, scaling a and b values for scipy.
		...
		if self.state.rule_applied==0:
			...
			self.state.boundaryscaled_split = ((self.data_loader.image_size-1)*self.state.split-self.state.x).astype(int)
		...
		if self.state.rule_applied==1:		
			# Transform to local patch coordinates.
			self.state.boundaryscaled_split = ((self.data_loader.image_size-1)*self.state.split-self.state.y).astype(int)
