...
class ActorModel():
	...
	def define_model(self, sess, model_file=None, to_train=None, name_scope=None, trainable_variables=True):
...
class CriticModel():
	...
	def define_model(self, sess, model_file=None, to_train=None, name_scope=None, actor_action=None):
...
class ActorCriticModel():
	...
	def __init__(self, sess, to_train=True):
		...
		self.actor_network.define_model(sess,to_train=to_train, name_scope='ActorModel')

		# with tf.name_scope('CriticModel') as scope:
		# with tf.variable_scope('CriticModel'):	
		self.critic_network = CriticModel()
		self.critic_network.define_model(sess,to_train=to_train, name_scope='CriticModel', actor_action=self.actor_network.predicted_split)

		self.target_network = CriticModel()
		self.target_network.define_model(sess,to_train=to_train, name_scope='TargetModel', actor_action=self.actor_network.predicted_split)
	...
	def define_critic_train_op(self):
		...
		self.target_variables = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope='TargetModel')
	...
	def define_actor_train_op(self):
		...
		self.actor_loss = -self.target_network.predicted_Qvalue
	...
	def define_update_target_operations(self):
		# Copy operation to update target network.
		self.copy_operation = [tf.assign(self.target_variables[var_index], self.critic_variables[var_index]) for var_index in range(len(self.target_variables))]
	...
	def create_network(self, sess, pretrained_weight_file=None, to_train=False):
		...
		self.define_update_target_operations()
