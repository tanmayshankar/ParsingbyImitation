...
class hierarchical():
	...
	def alter_backprop(self,image_index):

		j=0

		self.parse_tree[j].reward /= self.horrew[image_index].max()
		...
		self.attended_image = npy.zeros((self.image_size,self.image_size))
	...
	def update_plot_data(self, image_index):
		...
		# self.alternate_painted_image[npy.where(self.painted_image[image_index]==1)]=1.
	...
	def preprocess_images_labels(self):
		...
		self.num_images = len(self.hormax)

		self.images = self.images[self.hormax]
		self.true_labels = self.true_labels[self.hormax]
		self.original_images = self.original_images[self.hormax]
		self.images[npy.where(self.images==2)]=-1
		self.true_labels[npy.where(self.true_labels==2)]=-1
		self.horrew = self.horrew[self.hormax]
...
def parse_arguments():
	...
	parser.add_argument('--hormax',dest='hormax',type=str)
...
def main(args):
	...
	hierarchical_model.hormax = npy.load(args.hormax)
	hierarchical_model.horrew = npy.load(args.horrew)
