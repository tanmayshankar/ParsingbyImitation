...
class Parser():
	...
	def __init__(self, model_instance=None, data_loader_instance=None, memory_instance=None, plot_manager=None, args=None, session=None): 
		...
		self.initial_std_dev = 0.01
		self.final_std_dev = 0.01
		self.anneal_epochs = 50
		self.anneal_rate = (self.initial_std_dev-self.final_std_dev)/self.anneal_epochs
	...
	def set_parameters(self,e):
		if self.args.train:
			if e<self.anneal_epochs:
				self.covariance_value = self.initial_std_dev - self.anneal_rate*e
				self.annealed_epsilon = self.initial_epsilon-e*self.anneal_epsilon_rate
			else:
				self.covariance_value = self.final_std_dev
	...
	def process_splits_gaussian_behavioural_policy(self):
		# Earlier, sampling from epsilon greedy distribution constructed from the greedy split. 
		# Now, constructing a Gaussian distribution with mean as greedy_split, fixed covariance. 

		# Draw random samples from the multivariante normal. 
		# self.state.boundaryscaled_split = multivariate_normal.rvs(loc=self.greedy_split,cov=cov_val).astype(int)
		# Greedy split is from 1 to w-1 (or h-1). 

		if self.state.rule_applied==0:
			scale_val = self.state.w
		if self.state.rule_applied==1:
			scale_val = self.state.h
		
		redo = True
		while redo:
			self.state.split = multivariate_normal.rvs(mean=float(self.greedy_split)/scale_val,cov=self.covariance_value**2)			
		...
		if self.state.rule_applied==0:
			...
			# Convert to local patch coordinates.
	...
	def parse_nonterminal_offpolicy(self):
		...
		if self.state.rule_applied==0 or self.state.rule_applied==1:
			...
			self.process_splits_gaussian_behavioural_policy()
