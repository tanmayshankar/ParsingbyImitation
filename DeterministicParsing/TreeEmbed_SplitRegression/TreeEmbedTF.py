...
class Model():
	...
	def initialize_base_model(self, sess, model_file=None, to_train=None):
		...
		self.tree_embedding = tf.placeholder(tf.float32,shape=(None,92),name='TreeEmbedding')
		self.concat_input = tf.concat([self.fc7,self.tree_embedding],axis=-1)
		
		self.num_rules = 4
		
	def define_rule_stream(self):
		self.rule_fc8_shape = 200
		self.rule_fc9_shape = 100
		self.rule_fc8 = tf.layers.dense(self.concat_input,self.rule_fc8_shape,activation=tf.nn.relu)
		self.rule_fc9 = tf.layers.dense(self.rule_fc8,self.rule_fc9_shape,activation=tf.nn.relu)
	...
	def define_split_stream(self):
		...
		self.fc8 = tf.layers.dense(self.concat_input,self.fc8_shape,activation=tf.nn.relu)
		self.fc9 = tf.layers.dense(self.fc8,self.fc9_shape,activation=tf.nn.relu)
		self.sigmoid_split = tf.layers.dense(self.fc9,1,activation=tf.nn.sigmoid)
	...
	# 	self.normal_mean = tf.layers.dense(self.fc9,1)
	# 	self.normal_var = tf.layers.dense(self.fc9,1,activation=tf.nn.softplus)
