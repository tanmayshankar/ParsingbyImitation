...
class hierarchical():
	...
	def initialize_tensorflow_model(self, sess, model_file=None):
		...
		# Layer 1
		# Conv x Conv x Input Channels x Output Channels
		self.W_conv1 = tf.Variable(tf.truncated_normal([self.conv1_size,self.conv1_size, 2, self.conv1_num_filters],stddev=0.1),name='W_conv1')
	...
	def parse_nonterminal(self, image_index):
		...
		rule_probabilities = self.sess.run(self.rule_probabilities,feed_dict={self.input: self.resized_image.reshape(2,self.image_size,self.image_size,1)})
		...
		if selected_rule<=3:
			...
			if ((selected_rule==0) or (selected_rule==2)):
				...
				categorical_prob_softmax[:self.state.y]=0.
				categorical_prob_softmax[(self.state.y+self.state.h):]=0.
				categorical_prob_softmax = categorical_prob_softmax/categorical_prob_softmax.sum()
				
				while (split_location<=self.state.y)or(split_location>=(self.state.y+self.state.w)):

					split_location = npy.random.choice(range(20),p=categorical_prob_softmax)

					# if split_location>=(self.image_size/2):
			...
			if ((selected_rule==1) or (selected_rule==3)):
				...
				epsilon = 0.0001
				categorical_prob_softmax+=epislon
				categorical_prob_softmax[:self.state.x]=0.
				categorical_prob_softmax[(self.state.x+self.state.w):]=0.			
				categorical_prob_softmax = categorical_prob_softmax/categorical_prob_softmax.sum()
				
				# while (split_location<=0)or(split_location>=self.state.w):
				while (split_location<=self.state.x)or(split_location>=self.state.x+self.state.w):
	...
	def backprop(self, image_index, epoch):
		...
		for j in range(len(self.parse_tree)):
			...
			self.image_input = npy.zeros((self.image_size,self.image_size,2))
			self.image_input[:,:,0] = self.images[image_index]
			...
			self.image_input[:,:,1] = self.attended_image

			self.x_gradients = npy.diff(self.attended_image,axis=0)
			self.y_gradients = npy.diff(self.attended_image,axis=1)
			...
			if self.parse_tree[j].label==0:
				...
				merged_summaries, rule_loss, _ = self.sess.run([self.merge_summaries, self.rule_loss, self.train], \
					feed_dict={self.input: self.resized_image.reshape(2,self.image_size,self.image_size,1), self.rule_return_weight: rule_weight, \
