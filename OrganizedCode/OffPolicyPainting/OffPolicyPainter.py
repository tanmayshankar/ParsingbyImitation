...
class Parser():
	...
	def paint_terminal(self):
		# Label == 1 means paint. Label == 2 means don't paint.

		# Assign everything in the state to be not painted, then paint in a segment of paintbrush width.
		self.predicted_labels[self.state.image_index,self.state.x:self.state.x+self.state.w,self.state.y:self.state.y+self.state.h] = 2
		...
		if self.state.label==1:
			...
			if self.state.h<self.state.w: #This is opposite because of vertical horizontal switch in viz.
				...
				self.predicted_labels[self.state.image_index, self.state.x:(self.state.x+self.state.w), lower:upper] = 1.

			# Otherwise just use a horizontal stroke (== case evaluates to horizontal.)
			else: 
