...
#####################################################################################################
	...
	def build(self, sess, model_file=None):
		...
		with tf.device('/gpu:2'):
			...
			self.rule_fc_shapes = [[self.fc_input_shape,self.rule_num_hidden1,self.rule_num_hidden2,6]]			# Split stream
			...
			self.selected_rule_probabilities = self.rule_probabilities[0]
			...
			self.rule_loss = self.rule_loss_branch[0]
...
# May need to rewrite parse_nonterminal
	def parse_nonterminal(self, image_index):
		...
		greedy_index = npy.argmax(rule_probabilities[0])
		# print(len(rule_probabilities))
		rule_probabilities =  npy.ones(len(rule_probabilities[0]))*(self.annealed_epislon/len(rule_probabilities))
		# print("AFter:",len(rule_probabilities))
